...
class Feeder:
	...
	def _get_test_groups(self):
		...
		language_sequence = ' '.join(['1'] * len(text.split()))	## (shiyao): added here, new version of text-to-sequence requires a language seq

		input_data, _ = np.asarray(text_to_sequence(text, language_sequence, self._cleaner_names), dtype=np.int32)
