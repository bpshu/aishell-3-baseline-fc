from dca_synthesizer.utils.text import text_to_sequence
from dca_synthesizer.infolog import log
from dca_synthesizer.models import create_model
from dca_synthesizer.utils import plot
from dca_synthesizer import audio
...
class Tacotron2:
    ...
    def my_synthesize(self, speaker_embeds, texts):
        ...
        language_sequence = [' '.join(['1'] * len(text.split())) for text in texts]
        seqs = [np.asarray(text_to_sequence(text, language_sequence[idx], cleaner_names)[0]) for idx, text in enumerate(texts)]
